...
export default function Product({
  ...
  async function createproduct(ev) {
    ...
    try {
      if (_id) {
        await axios.put("/api/shops", { ...productData, _id });
        toast.success("product Updated");
      } else {
        await axios.post("/api/shops", productData);
  ...
  return (
    <form className="addWebsiteform" onSubmit={createproduct}>
      ...
      {/* Affiliate Link*/}
      <div className="w-100 flex flex-col flex-left mb-2">
        <label htmlFor="client">Affiliate Link</label>
        <input
          type="text"
          id="afilink"
      ...
      <div className="description w-100 flex flex-col flex-left mb-2">
        <label htmlFor="description">Enter Product Details</label>
        <MarkdownEditor
          value={description}
